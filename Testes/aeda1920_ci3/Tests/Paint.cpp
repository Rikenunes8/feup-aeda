...
//TODO
bool PaintCatalogItem::operator<(const PaintCatalogItem& pci) const {
    if (paintItem->getYear() == pci.getYear()) {
        if (paintItem->getAuthor() == pci.getAuthor()) {
            return paintItem->getTitle() < pci.getTitle();
        }
        else
            return paintItem->getAuthor() < pci.getAuthor();
    }
    else
        return paintItem->getYear() > pci.getYear();
}
